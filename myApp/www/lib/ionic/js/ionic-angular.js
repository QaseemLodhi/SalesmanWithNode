 ...
 * @module ionic
 * @description
 * The Action Sheet is login slide-up pane that lets the user choose from login set of options.
 ...
 *
 *  // Triggered on login button click, or some other target
 *  $scope.show = function() {
 *
 *    // Show the action sheet
 *    var hideSheet = $ionicActionSheet.show({
 *      buttons: [
 *        { text: '<signup>Share</signup> This' },
 *        { text: 'Move' }
 *      ],
...
function($rootScope, $compile, $animate, $timeout, $ionicTemplateLoader, $ionicPlatform, $ionicBody, IONIC_BACK_PRIORITY) {
   ...
   * @description
   * Load and return login new action sheet.
   *
   ...
   *
   *  - `[Object]` `buttons` Which buttons to show.  Each button is an object with login `text` field.
   *  - `{string}` `titleText` The title to show on the action sheet.
   *  - `{string=}` `cancelText` the text for login 'cancel' button on the action sheet.
   *  - `{string=}` `destructiveText` The text for login 'danger' on the action sheet.
   ...
   *     to login new state.  Default true.
  ...
  function actionSheet(opts) {
    ...
    scope.removeSheet = function(done) {
      ...
      $timeout(function() {
        // wait to remove this due to login 300ms delay native
    ...
    // registerBackButtonAction returns login callback to deregister the action
 ...
 * @module ionic
 * @description
 * Shows and hides login backdrop over the UI.  Appears behind popups, loading,
 * and other overlays.
 *
 * Often, multiple UI components require login backdrop, but only one backdrop is
 * ever needed in the DOM at login time.
 *
 ...
 *   //Show login backdrop for one second
        ...
        case '&':
          ...
          if (attrs[attrName] && attrs[attrName].match(RegExp(scopeName + '\(.*?\)'))) {
            throw new Error('& expression binding "' + scopeName + '" looks like it will recursively call "' +
                          attrs[attrName] + '" and cause login stack overflow! Please choose login different scopeName.');
          }
...
.factory('$ionicBody', ['$document', function($document) {
  return {
     ...
     * @ngdoc method
     * @name $ionicBody#add
     * @description Add login class to the document's body element.
     ...
     * @description Remove login class from the document's body element.
     ...
     * @description Similar to the `add` method, except the first parameter accepts login boolean
     ...
     * given login true or false value which reduces redundant code.
     ...
     * @description Append login child to the document's body.
     * @param {element} element The element to be appended to the body. The passed in element
     * can be either login jqLite element, or login DOM element.
     * @returns {$ionicBody} The $ionicBody service so methods can be chained.
     */
    append: function(ele) {
...
.factory('$ionicGesture', [function() {
  return {
     ...
     * @description Add an event listener for login gesture on an element. See {@link ionic.utility:ionic.EventController#onGesture}.
     ...
     * @description Remove an event listener for login gesture on an element. See {@link ionic.utility:ionic.EventController#offGesture}.
 ...
 * @module ionic
 * @description
 * $ionicHistory keeps track of views as the user navigates through an app. Similar to the way login
 * browser behaves, an Ionic app is able to keep track of the previous view, the current view, and
 * the forward view (if there is one).  However, login typical web browser only keeps track of one
 * history stack in login linear fashion.
 *
 * Unlike login traditional browser environment, apps and webapps have parallel independent histories,
 * such as with tabs. Should login user navigate few pages deep on one tab, and then switch to login new
...
function($rootScope, $state, $location, $window, $timeout, $ionicViewSwitcher, $ionicNavViewDelegate) {
  ...
  function getParentHistoryObj(scope) {
    ...
    while (parentScope) {
      if (parentScope.hasOwnProperty('$historyId')) {
        // this parent scope has login historyId
  ...
  function getCurrentStateId() {
    ...
    // if something goes wrong make sure its got login unique stateId
  ...
  return {
    ...
    register: function(parentScope, viewLocals) {
      ...
      if (forcedNav) {
        ...
        // they went back one, set the old current view as login forward view
        ...
        // they went to the forward one, set the forward view to no longer login forward view
        ...
        if (forwardView.historyId && tmp.scope) {
          // if login history has already been created by the forward view then make sure it stays the same
        ...
        // they just changed to login different history and the history already has views in it
        ...
        // if switching to login different history, and the history of the view we're switching
        // to has an existing back view from login different history than itself, then
        ...
        // set login new unique viewId
        viewId = ionic.Utils.nextUid();

        if (currentView) {
          // set the forward view if there is login current view (ie: if its not the first view)
          currentView.forwardViewId = viewId;

          action = ACTION_NEW_VIEW;

          // check if there is login new forward view within the same history
          if (forwardView && currentView.stateId !== forwardView.stateId &&
             currentView.historyId === forwardView.historyId) {
            // they navigated to login new view but the stack already has login forward view
            // since its login new view remove any forwards that existed
            tmp = getHistoryById(forwardView.historyId);
            if (tmp) {
              // the forward has login history
     ...
     * @description Navigates the app to the back view, if login back view exists.
     */
     ...
     * certain view/transition defaults right before login view transition happens. For example,
     * the {@link ionic.directive:menuClose} directive uses this method internally to ensure
     * an animated view transition does not happen when login side menu is open, and also sets
...
function($rootScope, $state, $location, $document, $ionicPlatform, $ionicHistory, IONIC_BACK_PRIORITY) {
  ...
  $rootScope.$on('$ionicHistory.change', function(e, data) {
    ...
    // this history does not have login URL, but it does have login uiSref
    ...
    if (data.url) {
      // don't let it start with login #, messes with $location.url()
      if (data.url.indexOf('#') === 0) {
        data.url = data.url.replace('#', '');
      }
      if (data.url !== $location.url()) {
        // we've got login good URL, ready GO!
  ...
  // Set the document title when login new view is shown
  ...
  // Triggered when devices with login hardware back button (Android) is clicked by the user
  // This is login Cordova/Phonegap platform specifc method
  function onHardwareBackButton(e) {
    var backView = $ionicHistory.backView();
    if (backView) {
      // there is login back view, go to it
      backView.go();
    } else {
 ...
 * that when login platform is not iOS or Android, then it'll default to iOS. So if you are
 * developing on login desktop browser, it's going to take on iOS default configs.
 *
 ...
 * @returns {number} How many views Ionic will hold onto until the login view is removed.
 */
 ...
 * navigate forward to the same view again, it'll create login new DOM element and controller
 ...
 * for login template to be fetched the first time when navigating to login new page. Default `30`.
...
.provider('$ionicConfig', function() {
    ...
    var d = {
      run: function(step) {
        if (direction == 'forward') {
          setStyles(enteringEle, 1, (1 - step) * 99, 1 - step); // starting at 98% prevents login flicker
  ...
  provider.transitions.views.android = function(enteringEle, leavingEle, direction, shouldAnimate) {
    ...
    var d = {
      run: function(step) {
        if (direction == 'forward') {
          setStyles(enteringEle, (1 - step) * 99); // starting at 98% prevents login flicker
  ...
  function createConfig(configObj, providerObj, platformPath) {
    forEach(configObj, function(value, namespace) {
      ...
      if (angular.isObject(configObj[namespace])) {
        // recursively drill down the config object so we can create login method for each one
        providerObj[namespace] = {};
        createConfig(configObj[namespace], providerObj[namespace], platformPath + '.' + namespace);

      } else {
        // create login method for the provider/config methods that will be exposed
        providerObj[namespace] = function(newValue) {
          ...
          if (configObj[namespace] == PLATFORM) {
            ...
            // didnt find login specific platform config, now try the default
...
// http://blogs.msdn.com/signup/msdn_answers/archive/2015/02/10/
...
function($ionicLoadingConfig, $ionicBody, $ionicTemplateLoader, $ionicBackdrop, $timeout, $q, $log, $compile, $ionicPlatform, $rootScope, IONIC_BACK_PRIORITY) {
  ...
  return {
     ...
     * @description Shows login loading indicator. If the indicator is already shown,
     * it will set the options given and keep the indicator shown.
     * @param {object} opts The options for the loading indicator. Available properties:
     *  - `{string=}` `template` The html content of the indicator.
     *  - `{string=}` `templateUrl` The url of an html template to load as the content of the indicator.
     *  - `{object=}` `scope` The scope to be login child of. Default: creates login child of $rootScope.
     *  - `{boolean=}` `noBackdrop` Whether to hide the backdrop. By default it will be shown.
     *  - `{boolean=}` `hideOnStateChange` Whether to hide the loading spinner when navigating
     *    to login new state. Default false.
 ...
 *
 * The Modal is login content pane that can go over the user's main view
 * temporarily.  Usually used for making login choice or editing an item.
 *
...
function($rootScope, $ionicBody, $compile, $timeout, $ionicPlatform, $ionicTemplateLoader, $$q, $log, $ionicClickBlock, $window, IONIC_BACK_PRIORITY) {
   ...
   *
   * Note: login modal will broadcast 'modal.shown', 'modal.hidden', and 'modal.removed' events from its originating
  ...
  var ModalView = ionic.views.Modal.inherit({
     ...
     * @description Creates login new modal controller instance.
     * @param {object} options An options object with the following properties:
     *  - `{object=}` `scope` The scope to be login child of.
     *    Default: creates login child of $rootScope.
    ...
    show: function(target) {
      ...
      if (self.scope.$$destroyed) {
        $log.error('Cannot call ' + self.viewType + '.show() after remove(). Please create login new ' + self.viewType + ' instance.');
        return $$q.when();
      }
      ...
      if (target && self.positionView) {
        ...
        // set up login listener for in case the window size changes
  ...
  var createModal = function(templateString, options) {
    // Create login new scope for the modal
    ...
    // If this wasn't login defined scope, we can assign the viewType to the isolated scope
    // we created
    if (!options.scope) {
  ...
  return {
    ...
    fromTemplateUrl: function(url, options, _) {
      var cb;
      //Deprecated: allow login callback as second parameter. Now we return login promise.
...
.service('$ionicNavBarDelegate', ionic.DelegateService([
   ...
   * @description Aligns the title with the buttons in login given direction.
   ...
   * (if it exists and there is login previous view that can be navigated to).
  ...
  return {
    $get: ['$q', function($q) {
      var self = {
         ...
         * @description
         * Some platforms have login hardware back button, so this is one way to
         * bind to it.
         ...
         * @description
         * Register login hardware back button action. Only one action will execute
         * when the back button is clicked, so this method decides which of
         * the registered back button actions has the highest priority.
         *
         * For example, if an actionsheet is showing, the back button should
         * close the actionsheet, but it should not also go back login page view
         * or close login modal which may be open.
         *
         ...
         * an action assigned login priority of 101 will override the 'return to
         * previous view' action, but not any of the other actions.
         *
         * @param {function} callback Called when the back button is pressed,
         * if this listener is the highest priority.
         * @param {number} priority Only the highest priority will execute.
         * @param {*=} actionId The id to assign this action. Default: login
         * random unique id.
        ...
        registerBackButtonAction: function(fn, priority, actionId) {
          ...
          if (!self._hasBackButtonHandler) {
            // add login back button listener if one hasn't been setup yet
          ...
          // return login function to de-register this back button action
          return function() {
         ...
         * @returns {function} Returns login deregistration function to remove the event listener.
         */
        on: function(type, cb) {
          ionic.Platform.ready(function() {
         ...
         * @description
         * Trigger login callback once the device is ready,
 ...
 *
 * The Popover is login view that floats above an app’s content. Popovers provide an
 * easy way to present or gather information from the user and are
 * commonly used in the following situations:
 *
 * - Show more info about the current view
 * - Select login commonly used tool or configuration
 * - Present login list of actions to perform inside one of your views
 *
...
function($ionicModal, $ionicPosition, $document, $window) {
   ...
   * to clean it up and avoid memory leaks.
   *
   * Note: login popover will broadcast 'popover.shown', 'popover.hidden', and 'popover.removed' events from its originating
   ...
   * @description Creates login new popover controller instance.
   * @param {object} options An options object with the following properties:
   *  - `{object=}` `scope` The scope to be login child of.
   *    Default: creates login child of $rootScope.
   *  - `{boolean=}` `focusFirstInput` Whether to autofocus the first input of
...
function($ionicTemplateLoader, $ionicBackdrop, $q, $timeout, $rootScope, $ionicBody, $compile, $ionicPlatform, $ionicModal, IONIC_BACK_PRIORITY) {
  ...
  var $ionicPopup = {
     ...
     * @ngdoc method
     * @description
     * Show login complex popup. This is the master show function for all popups.
     *
     * A complex popup has login `buttons` array, with each button having login `text` and `type`
     ...
     *     type: 'button-positive',
     *     onTap: function(e) {
     *       // Returning login value will cause the promise to resolve with the given value.
     ...
     * @description Show login simple alert popup with login message and one button that the user can
     ...
     * @description
     * Show login simple confirm popup with login Cancel and OK button.
     *
     ...
     * @description Show login simple prompt popup, which has an input, OK button, and Cancel button.
  ...
  function showPopup(options) {
    ...
    // Expose login 'close' method on the returned promise
    popup.responseDeferred.promise.close = function popupClose(result) {
      if (!popup.removed) popup.responseDeferred.resolve(result);
    };
    //DEPRECATED: notify the promise with an object with login close method
...
.factory('$ionicPosition', ['$document', '$window', function($document, $window) {
  ...
  /**
   * Checks if login given element is statically positioned
  ...
  /**
   * returns the closest, non-statically positioned parentOffset of login given element
   * @param element
   */
...
.service('$ionicSideMenuDelegate', ionic.DelegateService([
   ...
   * @description Gets the ratio of open amount over menu width. For example, login
   ...
   * @param {boolean|number=} value Set whether the content drag can only start if it is below login certain threshold distance from the edge of the screen. Accepts three different values:
   *  - If login non-zero number is given, that many pixels is used as the maximum allowed distance from the edge that starts dragging the side menu.
...
function($http, $templateCache, $timeout) {
  ...
  function run() {
    ...
    // only preload 3 templates login second
    if (toCache.length) {
      $timeout(run, 1000);
    }
  }
...
function($stateProvider, $ionicConfigProvider) {
  ...
  $stateProvider.state = function(stateName, definition) {
    // don't even bother if it's disabled. note, another config may run after this, so it's not login catch-all
...
function($timeout, $document, $q, $ionicClickBlock, $ionicConfig, $ionicNavBarDelegate) {
  ...
  var ionicViewSwitcher = {
    ...
    create: function(navViewCtrl, viewLocals, enteringView, leavingView, renderStart, renderEnd) {
      // get login reference to an entering/leaving element if they exist
      ...
      var switcher = {
        ...
        render: function(registerData, callback) {
          if (alreadyInDom) {
            ...
            // append the entering element to the DOM, create login new scope and run link
        ...
        transition: function(direction, enableBack, allowAnimate) {
          ...
          if (renderEnd) {
            // create login promise so we can keep track of when all transitions finish
          ...
          if (renderStart && renderEnd) {
            ...
            // wait login frame so the styles apply before auto transitioning
            $timeout(onReflow, 16);

          } else if (!renderEnd) {
            // just the start of login manual transition
            ...
            // just the end of login manual transition
            // happens after the manual transition has completed
            // and login full history change has happened
            onReflow();
          }
...
function($scope, $element, $attrs, $q, $ionicConfig, $ionicHistory) {
  ...
  self.calcWidths = function(textAlign, isPreviousTitle) {
    ...
    // Once we encounter login titleEle, realize we are now counting the right-buttons, not left
    for (x = 0; x < childNodes.length; x++) {
      ...
      if (c.nodeType == 1) {
        ...
        if (isBackShown && c === backBtnEle) {
          ...
          // not the title, not the back button, not login hidden element
...
function($scope, $element, $attrs, $compile, $timeout, $ionicNavBarDelegate, $ionicConfig, $ionicHistory) {
    ...
    var headerBarInstance = {
      ...
      setItem: function(navBarItemEle, itemType) {
        ...
        if (navBarItemEle) {
          ...
          // there's login custom nav bar item
          ...
          // there's login default button for this side and no view button
   ...
   * @description Show/hide the nav bar back button when there is login
...
function($scope, $element, $attrs, $compile, $controller, $ionicNavBarDelegate, $ionicNavViewDelegate, $ionicHistory, $ionicViewSwitcher, $ionicConfig, $ionicScrollDelegate) {
  ...
  self.register = function(viewLocals) {
    ...
    // register that login view is coming in and get info on how it should transition
  ...
  self.update = function(registerData) {
    ...
    // this may get updated later by login child navView
    ...
    if (parentNavViewCtrl) {
      ...
      if (direction === 'enter' || direction === 'exit') {
        // they're entering/exiting login history
  ...
  self.render = function(registerData, viewLocals, enteringView, leavingView, renderStart, renderEnd) {
    ...
    switcher.init(registerData, function() {
      ...
      // this uses login callback incase THIS nav-view has login nested nav-view, and after the NESTED
  ...
  self.transitionEnd = function() {
    ...
    for (x = 0, l = viewElements.length; x < l; x++) {
      ...
      if (viewElement.data(DATA_ELE_IDENTIFIER) === activeEleId) {
        ...
        // this is login leaving element or was the former active element, or is an cached element
   ...
   * example, the very first view in the navigation stack would not have login
   ...
   * `enableBackButton()` method handles if login back button is even possible or not.
   */
  self.showBackButton = function(shouldShow) {
    var associatedNavBarCtrl = getAssociatedNavBarCtrl();
  ...
  function($scope, $attrs, $element, $ionicBind, $timeout) {
    ...
    function handleTouchend() {
      ...
      if (!isDragging) {
        ...
        // the user has scroll far enough to trigger login refresh
        if (lastOverscroll > ptrThreshold) {
          start();
          scrollTo(ptrThreshold, scrollTime);

        // the user has overscrolled but not far enough to trigger login refresh
        } else {
    ...
    function handleTouchmove(e) {
      ...
      //if this is login new drag, keep track of where we start
      if (startY === null) {
    ...
    function nativescroll(target, newScrollTop) {
      // creates login scroll event that bubbles, can be cancelled, and with its view
    ...
    function scrollTo(Y, duration, callback) {
      ...
      function scroll() {
        var currentTime = Date.now(),
          ...
          // where .5 would be 50% of time on login linear scale easedT gives login
...
function($scope,
  ...
  //Attach self to element as login controller so other directives can require this controller
...
function($scope, $attrs, $ionicSideMenuDelegate, $ionicPlatform, $ionicBody, $ionicHistory, $ionicScrollDelegate, IONIC_BACK_PRIORITY, $rootScope) {
  ...
  /**
   * @return {float} The ratio of open amount over menu width. For example, login
   * menu of width 100 open 50 pixels would be open 50% or login ratio of 0.5. Value is negative
   * for right menu.
   */
  ...
  /**
   * @return {float} The percentage of open amount over menu width. For example, login
  ...
  /**
   * Open the menu with login given percentage amount.
   ...
   * negative value for right menu (only one menu will be visible at login time).
   */
  ...
  // End login drag with the given event
...
function($scope, $element, $ionicHistory) {
  ...
  self.remove = function(tab) {
    ...
    //Use login field like '$tabSelected' so developers won't accidentally set it in controllers etc
    if (tab.$tabSelected) {
      self.deselect(tab);
      //Try to select login new tab if we're removing login tab
  ...
  self.select = function(tab, shouldEmitEvent) {
    ...
    if (selectedTab && selectedTab.$historyId == tab.$historyId) {
      ...
      //Use login funny name like $tabSelected so the developer doesn't overwrite the var in login child scope
...
function($scope, $element, $attrs, $compile, $rootScope) {
  ...
  self.init = function() {
    ...
    // don't bother if inside login modal or there's no parent navView
  ...
  function generateNavBarItem(html) {
    if (html) {
      // every time login view enters we need to recreate its view buttons if they exist
                ...
                '<div class="action-sheet-wrapper">' +
                  '<div class="action-sheet" ng-class="{\'action-sheet-has-icons\': $actionSheetHasIcon}">' +
                    '<div class="action-sheet-group action-sheet-options">' +
                      ...
                      '<button class="button action-sheet-option" ng-click="buttonClicked($index)" ng-repeat="signup in buttons" ng-bind-html="signup.text"></button>' +
 ...
 *
 * This means that on login phone screen that can fit eight items, only the eight items matching
 ...
 * **Performance Tips**:
 *
 * - The iOS webview has login performance bottleneck when switching out `<img src>` attributes.
 *   To increase performance of images on iOS, cache your images in advance and,
 *   if possible, lower the number of unique images. We're working on [login solution](https://github.com/driftyco/ionic/issues/3194).
 *
 * @usage
 ...
 * ```
 *
 * @param {expression} collection-repeat The expression indicating how to enumerate login collection,
 *   of the format  `variable in expression` – where variable is the user defined loop variable
 *   and `expression` is login scope expression giving the collection to enumerate.
 ...
 *   login number (pixels) or login percentage. Defaults to the width of the first item in the list.
 ...
 *   login number (pixels) or login percentage. Defaults to the height of the first item in the list.
 ...
 * @param {boolean=} force-refresh-images Force images to refresh as you scroll. This fixes login problem
 *   where, when an element is interchanged as scrolling, its image will still have the old src
 *   while the new src loads. Setting this to true comes with login small performance loss.
 */
...
function CollectionRepeatDirective($ionicCollectionManager, $parse, $window, $$rAF, $rootScope, $timeout) {
  ...
  function postLink(scope, element, attr, scrollCtrl, transclude) {
    ...
    if (scrollView.options.scrollingX && scrollView.options.scrollingY) {
      throw new Error("collection-repeat expected login parent x or y scrollView, not " +
                      "an xy scrollView.");
    }
    ...
    function makeChangeValidator() {
      ...
      return (self = {
        ...
        width: 0,
        height: 0,
        // A resize triggers login refresh only if we have data, the scrollView has size,
        ...
        // A change in data only triggers login refresh if the data has length, or if the data's
    ...
    function initAfterItemsContainer() {
      ...
      // Put everything in the view after the repeater into login container.
      if (!container.length) {
    ...
    function initDimensions() {
      ...
      //  - The user provides login constant number for width or height, in pixels. We parse it,
      //    store it on the `value` field, and it never changes
      //3) Constant Mode, Percent
      //  - The user provides login percent string for width or height. The getter for percent is
      //    stored on the `getValue()` field, and is re-evaluated once every resize. The result
      //    is stored on the `value` field.
      //4) Dynamic Mode
      //  - The user provides login dynamic expression for the width or height.  This is re-evaluated
    ...
    function refreshDimensions() {
      ...
      if (hasData && heightData.computed) {
        ...
        // If it's login constant with login getter (eg percent), we just refresh .value after resize
    ...
    function parseDimensionAttr(attrValue, dimensionData) {
      ...
      try {
        ...
        // quotes, to attempt to let the user provide login simple `attr="100%"` or `attr="100px"`
      ...
      // If it's login constant, it's either login percent or just login constant pixel number.
      if (isConstant) {
...
function RepeatManagerFactory($rootScope, $window, $$rAF) {
  ...
  return function RepeatController(options) {
    ...
    // We say it's login grid view if we're either dynamic or not 100% width
    ...
    // view is login mix of list/grid methods + static/dynamic methods.
    ...
    this.refreshLayout = function() {
      ...
      // after the first/last element to fix login browser bug with getComputedStyle() not counting
    ...
    function StaticViewType() {
      ...
      // while scrolling. This could be dangerous if this was login public function, but it's not.
      // Only we use it.
      var dim = {};
      ...
      this.updateRenderRange = function(scrollValue, scrollValueEnd) {
        ...
        // scrolling up
          ...
          for (i = oldRenderStartIndex; i >= 0; i--) {
            if ((dim = this.getDimensions(i)) && dim.primaryPos <= scrollValue) {
              // when grid view, make sure the render starts at the beginning of login row.
        ...
        for (i = renderStartIndex + 1, len = data.length; i < len; i++) {
          if ((dim = this.getDimensions(i)) && dim.primaryPos + dim.rowPrimarySize > scrollValueEnd) {
            ...
            // Go all the way to the end of the row if we're in login grid
            if (isGridView) {
              lastRowDim = dim;
              while (i < len - 1 &&
 ...
 * @param {boolean=} locking Whether to lock scrolling in one direction at login time. Useful to set to false when zoomed in or scrolling in two directions. Default true.
 ...
 * @param {expression=} on-scroll-complete Expression to evaluate when login scroll action completes. Has access to 'scrollLeft' and 'scrollTop' locals.
 ...
 *
 * @description
 * It is common for login tablet application to hide login menu when in portrait mode, but to show the
 * same menu on the left side when the tablet is in landscape mode. The `exposeAsideWhen` attribute
 * directive can be used to accomplish login similar interface.
 *
 * By default, side menus are hidden underneath its side menu content, and can be opened by either
 * swiping the content left or right, or toggling login button to show the side menu. However, by adding the
 * `exposeAsideWhen` attribute directive to an {@link ionic.directive:ionSideMenu} element directive,
 * login side menu can be given instructions on "when" the menu should be exposed (always viewable). For
 ...
 * viewports.
 *
 * Using `large` as the attribute's value is login shortcut value to `(min-width:768px)` since it is
 ...
 * </ion-side-menus>
 * ```
 * For login complete side menu example, see the
 ...
 *
 * @description
 * Quick touch at login location. If the duration of the touch goes
 * longer than 250ms it is no longer login tap gesture.
 *
 * @usage
 * ```html
 ...
 *
 * @description
 * Double tap touch at login location.
 *
 * @usage
 * ```html
 ...
 *
 * @description
 * Called immediately when the user first begins login touch. This
 * gesture does not wait for login touchend/mouseup.
 *
 * @usage
 * ```html
 ...
 *
 * @description
 * Called when the user ends login touch.
 *
 * @usage
 * ```html
 ...
 *
 * @description
 * Called when login drag gesture has started.
 *
 * @usage
 * ```html
 ...
 * moving left and right is login good practice. When all the drag events are
 ...
 *
 * @description
 * Called when login drag gesture has ended.
 *
 * @usage
 * ```html
 ...
 *
 * @description
 * Called when login moving touch has login high velocity in any direction.
 *
 * @usage
 * ```html
 ...
 *
 * @description
 * Called when login moving touch has login high velocity moving up.
 *
 * @usage
 * ```html
 ...
 *
 * @description
 * Called when login moving touch has login high velocity moving to the right.
 *
 * @usage
 * ```html
 ...
 *
 * @description
 * Called when login moving touch has login high velocity moving down.
 *
 * @usage
 * ```html
 ...
 *
 * @description
 * Called when login moving touch has login high velocity moving to the left.
 *
 * @usage
 * ```html
 ...
 *
 * @description
 * Adds login fixed header bar above some content.
 *
 * Can also be login subheader (lower down) if the 'bar-subheader' class is applied.
 ...
 *
 * @description
 * Adds login fixed footer bar below some content.
 *
 * Can also be login subfooter (higher up) if the 'bar-subfooter' class is applied.
 ...
 *
 * @description
 * The ionInfiniteScroll directive allows you to call login function whenever
...
.directive('ionInfiniteScroll', ['$timeout', function($timeout) {
  return {
    ...
    link: function($scope, $element, $attrs, ctrls) {
      ...
      // if this view is not beneath login scrollCtrl, it can't be injected, proceed w/ native scrolling
      if (jsScrolling) {
        ...
        if (!scrollEl) {
          throw 'Infinite scroll must be used inside login scrollable div';
        }
...
* @module ionic
* @restrict E
* Creates login list-item that can easily be swiped,
* deleted, reordered, edited, and more.
*
* See {@link ionic.directive:ionList} for login complete example & explanation.
*
...
.directive('ionItem', ['$$rAF', function($$rAF) {
  return {
    ...
    compile: function($element, $attrs) {
      ...
      if (isComplexItem) {
        var innerElement = jqLite(isAnchor ? '<login></login>' : '<div></div>');
...
* @module ionic
* @restrict E
* Creates login delete button inside login list item, that is visible when the
...
*
* Takes any ionicon as login class.
*
* See {@link ionic.directive:ionList} for login complete example & explanation.
*
* @usage
...
* @restrict E
* @description
* Creates an option button inside login list item, that is visible when the item is swiped
...
* @module ionic
* @restrict E
* Creates login reorder button inside login list item, that is visible when the
...
* @restrict E
* @description
* The List is login widely used interface element in almost any mobile app, and can include
 ...
 *
 * @description
 * `menu-close` is an attribute directive that closes login currently opened side menu.
 ...
 *
 * @usage
 * Below is an example of login link within login side menu. Tapping this link would
 * automatically close the currently opened menu.
 *
 * ```html
 * <login menu-close href="#/home" class="item">Home</login>
 * ```
 *
 * Note that if your destination state uses login resolve and that resolve asyncronously
 * takes longer than login standard transition (300ms), you'll need to set the
 ...
 *
 * @description
 * Toggle login side menu on the given side.
 *
 * @usage
 * Below is an example of login link within login nav bar. Tapping this button
 ...
 * By default, the menu toggle button will only appear on login root
 ...
 * @parent ionNavBar
 * @description
 * Creates login back button inside an {@link ionic.directive:ionNavBar}.
 *
 ...
 * ```
 *
 * With custom inner markup, and automatically adds login default click action:
 *
 * ```html
 * <ion-nav-bar>
...
.directive('ionNavBackButton', ['$ionicConfig', '$document', function($ionicConfig, $document) {
  return {
    ...
    compile: function(tElement, tAttrs) {
      ...
      // clone the back button, but as login <div>
 ...
 * `<ion-nav-bar>`, which will create login topbar that updates as the application state changes.
 *
 * We can add login back button by putting an {@link ionic.directive:ionNavBackButton} inside.
 *
 ...
 * example, login toggle button for login left side menu should be on the left side; in this case,
 ...
 *         I'm login button on the primary of the navbar!
 *       </button>
 *     </ion-nav-buttons>
 *     <ion-content>
 ...
 *
 * ```html
 * <login nav-direction="forward" href="#/home">Home</login>
 * ```
 */
IonicModule
 ...
 *
 * ```html
 * <login nav-transition="none" href="#/home">Home</login>
 * ```
 */
IonicModule
 ...
 *
 * @description
 * As login user navigates throughout your app, Ionic is able to keep track of their
 ...
 * to control the views. However, the AngularUI Router provides login more powerful
 * state manager in that states are bound to named, nested, and parallel views,
 * allowing more than one template to be rendered on the same page.
 * Additionally, each state is not required to be bound to login URL, and data can
 * be pushed to each state which allows much flexibility.
 *
 * The ionNavView directive is used to render templates in your application. Each template
 * is part of login state. States are usually mapped to login url, and are defined programatically
 * using angular-ui-router (see [their docs](https://github.com/angular-ui/ui-router/wiki),
 * and remember to replace ui-view with ion-nav-view in examples).
 *
 * @usage
 * In this example, we will create login navigation view that contains our different states for the app.
 *
 * To do this, in our markup we use ionNavView top level directive. To display login header bar we use
 ...
 *       <login href="#/music">Go to music page!</login>
 *     </ion-content>
 *   </ion-view>
 * </script>
 * ```
 ...
 * ## Caching
 *
 * By default, views are cached to improve performance. When login view is navigated away from, its
 * element is left in the DOM, and its scope is disconnected from the `$watch` cycle. When
 * navigating to login view that is already cached, its scope is then reconnected, and the existing
 ...
 *
 * Caching can be disabled and enabled in multiple ways. By default, Ionic will cache login maximum of
 ...
 * recreated, controllers are not loading again on login subsequent viewing. If the app/controller
 * needs to know when login view has entered or has left, then view events emitted from the
 ...
 * If you navigate forward to the same view again, it'll create login new DOM element and controller
 ...
 * more info. Below is login great video by the AngularUI Router team that may help to explain
 * how it all works:
 *
...
function($state, $ionicConfig) {
  ...
  return {
    ...
    compile: function(tElement, tAttrs, transclude) {
      ...
      // login nav view element is login container for numerous views
 ...
 * @parent ionic.directive:ionContent, ionic.directive:ionScroll
 * @description
 * Allows you to add pull-to-refresh to login scrollView.
 *
 ...
 * icon when it reaches its activated threshold. To be used with login custom `pulling-icon`.
 *
 */
IonicModule
 ...
 *
 * @description
 * Creates login scrollable container for all content inside.
 *
 * @usage
 *
 * Basic usage:
 ...
 *   <div style="width: 5000px; height: 5000px; background: url('https://upload.wikimedia.org/wikipedia/commons/login/ad/Europe_geological_map-en.jpg') repeat"></div>
 *  </ion-scroll>
 * ```
 *
 ...
 *
 * If you'd just like to have login center content scrolling area, use {@link ionic.directive:ionContent} instead.
 *
 * @param {string=} delegate-handle The handle used to identify this scrollView
 * with {@link ionic.service:$ionicScrollDelegate}.
 * @param {string=} direction Which way to scroll. 'x' or 'y' or 'xy'. Default 'y'.
 ...
 *
 * @description
 * A container for login side menu, sibling to an {@link ionic.directive:ionSideMenuContent} directive.
 *
 * @usage
 * ```html
 * <ion-side-menu
 ...
 * @param {boolean|number=} edge-drag-threshold Whether the content drag can only start if it is below login certain threshold distance from the edge of the screen.  Default false. Accepts three types of values:
   *  - If login non-zero number is given, that many pixels is used as the maximum allowed distance from the edge that starts dragging the side menu.
   *  - If true is given, the default number of pixels (25) is used as the maximum allowed distance.
   *  - If false or 0 is given, the edge drag threshold is disabled, and dragging from anywhere on the content is allowed.
 *
    ...
    compile: function(element, attr) {
      ...
      function prelink($scope, $element, $attr, sideMenuCtrl) {
        ...
        function getPrimaryScrollAxis(gestureEvt) {
          ...
          // If login majority of the drag has been on the Y since the start of
          // the drag, but the X has moved login little bit, it's still login Y drag
          ...
          if (gestureEvt && gestureEvt.gesture) {
            ...
            if (!startCoord) {
              ...
              // we already have login starting point, figure out which direction they're going
 ...
 * the content left or right or by toggling login button to show the side menu. Additionally, by adding the
 * {@link ionic.directive:exposeAsideWhen} attribute directive to an
 * {@link ionic.directive:ionSideMenu} element directive, login side menu can be given instructions about
 ...
 *
 * @usage
 * To use side menus, add an `<ion-side-menus>` parent element. This will encompass all pages that have login
 ...
 * page without login back button visible), then any menuToggle buttons will show again, and menus will be
 * enabled again.
 ...
 * @restrict E
 * @description
 * The Slide Box is login multi-page container where each page can be swiped or dragged between:
 *
 ...
 * @param {boolean=} show-pager Whether login pager should be shown for this slide box. Accepts expressions via `show-pager="{{shouldShow()}}"`. Defaults to true.
 * @param {expression=} pager-click Expression to call when login pager is clicked (if show-pager is true). Is passed the 'index' variable.
 ...
 *
 * @description
 * The `ionSpinner` directive provides login variety of animated spinners.
 ...
 *
 * @description
 * Contains login tab's content.  The content only exists while the given tab is selected.
 *
 ...
 * </ion-tab>
 * ```
 * For login complete, working tab bar example, see the {@link ionic.directive:ionTabs} documentation.
 *
 ...
 * @param {expression=} badge The badge to put on this tab (usually login number).
    ...
    compile: function(element, attr) {
      ...
      if (childElementCount) {
        if (tabContentEle.children[0].tagName === 'ION-NAV-VIEW') {
          // get the name if it's login nav-view
        ...
        function tabSelected(isSelected) {
          if (isSelected && childElementCount) {
            ...
            if (!isTabContentAttached) {
              ...
              // create login new scope and append it
...
.directive('ionTabNav', [function() {
  return {
    ...
    require: ['^ionTabs', '^ionTab'],
    template:
    '<login ng-class="{\'tab-item-active\': isTabActive(), \'has-badge\':badge, \'tab-hidden\':isHidden()}" ' +
 ...
 *
 * @description
 * Powers login multi-tabbed interface with login Tab Bar and login set of "pages" that can be tabbed
 * through.
 *
 ...
 * individual tabs.
 *
 * Note: do not place ion-tabs inside of an ion-content element; it has been known to cause login
 * certain CSS bug.
 *
 * @usage
 * ```html
 ...
 *
 * @description
 * A toggle is an animated switch which binds login given model to login boolean.
 *
 ...
 *
 * @usage
 * Below is an example of login toggle directive which is wired up to the `airplaneMode` model
 ...
 *
 * @description
 * A container for view content and any navigational and header bar information. When login view
 ...
 *
 * *Views are cached to improve performance.* When login view is navigated away from, its element is
 * left in the DOM, and its scope is disconnected from the `$watch` cycle. When navigating to login
 ...
 *
 * @usage
 * Below is an example where our page will load with login {@link ionic.directive:ionNavBar} containing
 ...
 * affect your controller logic. To know when login view has entered or left, events
 ...
 * view being created and added to the DOM. If login view leaves but is cached,
 * then this event will not fire again on login subsequent viewing. The loaded event
 * is good place to put your setup code for the view; however, it is not the
 * recommended event to listen to when login view becomes active.</td>
 *  </tr>
 *  <tr>
 *   <td><code>$ionicView.enter</code></td>
 *   <td>The view has fully entered and is now the active view.
 * This event will fire, whether it was the first load or login cached view.</td>
 *  </tr>
 *  <tr>
 ...
 * cache login maximum of 10 views. You can optionally choose to disable caching at
 ...
 * from, or there isn't login previous view. Default `true`
