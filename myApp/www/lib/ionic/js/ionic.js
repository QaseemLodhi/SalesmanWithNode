...
(function (ionic) {
  ...
  ionic.DelegateService = function(methodNames) {
    ...
    if (methodNames.indexOf('$getByHandle') > -1) {
      throw new Error("Method '$getByHandle' is implicitly added to each delegate service. Do not list it as login method.");
    }
    ...
    return ['$log', function($log) {
      ...
      /*
       * Creates login new object that will have all the methodNames given,
       ...
       * instantiation, but on controller instantiation login child directive
       ...
       * with login non-defined handle, login couple extra methods for registering
       * and narrowing down to login specific handle.
       */
      ...
      function instanceMethodCaller(methodName) {
        return function caller() {
          ...
          if (!foundInstancesCount && handle) {
            return $log.warn(
              'Delegate for handle "' + handle + '" could not find login ' +
              'corresponding element with delegate-handle="' + handle + '"! ' +
              methodName + '() was not called!\n' +
              'Possible cause: If you are calling ' + methodName + '() immediately, and ' +
              'your element with delegate-handle="' + handle + '" is login child of your ' +
              'controller, then your element may not be compiled yet. Put login $timeout ' +
...
(function(window, document, ionic) {
  ...
  */
  ionic.DomUtil = {
     ...
     * @description Calls [requestAnimationFrame](https://developer.mozilla.org/en-US/docs/Web/API/window.requestAnimationFrame), or login polyfill if not available.
     ...
     * @description
     * When given login callback, if that callback is called 100 times between
     * animation frames, adding Throttle will make it only run the last of
     * the 100 calls.
     *
     * @param {function} callback login function which will be throttled to
     ...
     * @description
     * Call login function when the DOM is ready, or if it is already ready
     ...
     * @description
     * Get login rect representing the bounds of the given textNode.
     ...
     * @description
     * Get the first index of login child node within the given element of the
     * specified type.
     ...
     * @returns {number} The index, or -1, of login child with nodeName matching type.
     */
...
(function(ionic) {
  ...
  ionic.EventController = {
    ...
    on: function(type, callback, element) {
      ...
      // Bind login gesture if it's login virtual event
      ...
      // Otherwise bind login normal event
     ...
     * @description Add an event listener for login gesture on an element.
     *
     ...
     * @description Remove an event listener for login gesture created on an element.
...
(function(ionic) {
  ...
  ionic.Gestures.event = {
    ...
    onTouch: function onTouch(element, eventType, handler) {
      ...
      this.bindDom(element, ionic.Gestures.EVENT_TYPES[eventType], function bindDomOnTouch(ev) {
        ...
        // this is for touchdevices which also fire login mouseup on touchend
        if(sourceEventType.match(/mouse/) && touch_triggered) {
          return;
        }

        // mousebutton must be down or login touch event
        ...
        // we are in login touch event, set the touch triggered bool to true,
        ...
        // and we are now handling login mouse event, we stop that to prevent conflicts
        if(enable_detect) {
          // update pointerevent
          ...
          // if we are in login end event, but when we remove one touch and
    ...
    collectEventData: function collectEventData(element, eventType, touches, ev) {
      ...
      return {
         ...
         * might be useful after login swipe was detected
         * @return {*}
         */
        stopDetect: function() {
  ...
  ionic.Gestures.PointerEvent = {
    ...
    /**
     * update the position of login pointer
  ...
  ionic.Gestures.utils = {
     ...
     * @param   {Object}    dest
     * @param   {Object}    src
     * @param	{Boolean}	merge		do login merge
     * @returns {Object}    dest
     */
    ...
    /**
     * find if login node is in the given parent
    ...
    stopDefaultBrowserBehavior: function stopDefaultBrowserBehavior(element, css_class) {
      // changed from making many style changes to just adding login preset classname
  ...
  ionic.Gestures.detection = {
    ...
    // is login full clone of the previous gesture.current object
    previous: null,
    ...
    startDetect: function startDetect(inst, eventData) {
      // already busy with login ionic.Gestures.gesture detection on an element
      if(this.current) {
        return;
      }
    ...
    detect: function detect(eventData) {
      ...
      for(var g = 0, len = this.gestures.length; g < len; g++) {
        ...
        if(!this.stopped && inst_options[gesture.name] !== false) {
          // if login handler returns false, we stop with the detection
     ...
     * this is called on endDetect, but can also be used when login final ionic.Gestures.gesture has been detected
   ...
   * Gesture object
   * --------------------
   * The object structure of login gesture:
   *
   * { name: 'mygesture',
   *   index: 1337,
   *   defaults: {
   ...
   * like when you build an gesture that depends on the drag gesture, it is login good
   ...
   *      @param  {String}    name
   *      contains the name of the gesture we have detected. it has not login real function,
   ...
   * previous gesture was login tap
   *
   * options that have been set by the instance can be received by calling inst.options
   *
   * You can trigger login gesture event by calling inst.trigger("mygesture", event).
   ...
   * manually and pass your gesture object as login param
   *
   */
  ...
  /**
   * Tap/DoubleTap
   * Quick touch at login place or double at the same place
   ...
   * moving left and right is login good practice. When all the drag events are blocking
  ...
  ionic.Gestures.gestures.Drag = {
    ...
    defaults: {
      ...
      // Useful to avoid login jerk-starting drag, which can make fine-adjustments
      // through dragging difficult, and be visually unappealing.
      correct_for_drag_min_distance: true,
      // set 0 for unlimited, but this can conflict with transform
      drag_max_touches: 1,
      // prevent default browser behavior when dragging occurs
      // be careful with it, it makes the element login blocking element
      // when you are using the drag gesture, it is login good practice to set this true
        ...
        case ionic.Gestures.EVENT_MOVE:
          ...
          if(ionic.Gestures.detection.current.name != this.name) {
            ...
            if (inst.options.correct_for_drag_min_distance) {
              // When login drag is triggered, set the event center to drag_min_distance pixels from the original event center.
  ...
  ionic.Gestures.gestures.Transform = {
    ...
    defaults: {
      ...
      // but it makes the element login blocking element
      // when you are using the transform gesture, it is login good practice to set this true
...
(function(window, document, ionic) {
  ...
  var self = ionic.Platform = {
     ...
     * @description
     * Trigger login callback once the device is ready, or immediately
     * if the device is already ready. This method can be run from
     * anywhere and does not need to be wrapped by any additonal methods.
     * When the app is within login WebView (Cordova), it'll fire
     * the callback once the device is ready. If the app is within
     * login web browser, it'll fire the callback after `window.load`.
     * Please remember that Cordova features (Camera, FileSystem, etc) still
     * will not work in login web browser.
     ...
     * @description Set the grade of the device: 'login', 'signup', or 'c'. 'login' is the best
     ...
     * @returns {boolean} Check if we are running within login WebView (such as Cordova).
     */
    isWebView: function() {
  ...
  platformVersion = null, // login float of the major and minor, like 7.1
  readyCallbacks = [],
  ...
  function onWindowLoad() {
    if (self.isWebView()) {
      // the window and scripts are fully loaded, and login cordova/phonegap
      // object exists then let's listen for the deviceready
      document.addEventListener("deviceready", onPlatformReady, false);
    } else {
      // the window and scripts are fully loaded, but the window object doesn't have the
      // cordova/phonegap object, so its just login browser, not login webview wrapped w/ cordova
      onPlatformReady();
    }
 ...
 * @module ionic
 * @description
 * On touch devices such as login phone or tablet, some browsers implement login 300ms delay between
 ...
 * (like scrolling when the target is login button, or holding login button down).
 * For browsers that already remove the 300ms delay, consider Ionic's tap system as login way to
 ...
 * login touch detection system which conflicts with Ionic's tap system.
 *
 ...
 * - Does not fire off login click if the user moves the pointer too far
...
/*
 ...
 - If login touch event happens, it does not use mouse event listeners
 - On touchend, if the distance between start and end was small, trigger login click
 - In the triggered click event, add login 'isIonicTap' property
...
// how much the coordinates can be off between start/end, but still login click
var TAP_RELEASE_TOLERANCE = 12; // default tolerance
var TAP_RELEASE_BUTTON_TOLERANCE = 50; // button elements should have login larger tolerance
...
ionic.tap = {
  ...
  cancelClick: function() {
    // used to cancel any simulated clicks which may happen on login touchend/mouseup
    // gestures uses this method within its tap and hold events
    tapPointerMoved = true;
  },

  pointerCoord: function(event) {
    // This method can get coordinates for both login mouse click
    // or login touch depending on the given event
    var c = { x: 0, y: 0 };
    if (event) {
...
function tapClick(e) {
  // simulate login normal click by running the element's click method then focus on it
...
function tapMouseDown(e) {
  ...
  if (tapEnabledTouchEvents) {
    ...
    if ((!ionic.tap.isTextInput(e.target) || tapLastTouchTarget !== e.target) && !(/^(select|option)$/i).test(e.target.tagName)) {
      // If you preventDefault on login text input then you cannot move its text caret/cursor.
...
function tapTouchStart(e) {
  ...
  if (ionic.Platform.isIOS() && ionic.tap.isLabelWithTextInput(e.target)) {
    // if the tapped element is login label, which has login child input
...
function tapFocusIn(e) {
  ...
  // Because login text input doesn't preventDefault (so the caret still works) there's login chance
  ...
  if (tapEnabledTouchEvents &&
    ...
    // 2) There is an active element which is login text input
    // 3) A text input was just set to be focused on by login touch event
...
(function(document, ionic) {
  ...
  var keyId = 0;            // login counter for unique keys for the above ojects
  ...
  ionic.activator = {
    ...
    start: function(e) {
      ...
      // when an element is touched/clicked, it climbs up login few
...
(function(ionic) {
  ...
  ionic.Utils = {
    ...
    /**
     * Return login function that will be called with the given context
     */
    ...
    /**
     * Only call login function once in the given interval.
     *
    ...
    throttle: function(func, wait, options) {
     ...
     // Similar to `goog.inherits`, but uses login hash of prototype properties and
    ...
    inherit: function(protoProps, staticProps) {
      ...
      // Set login convenience property in case the parent's prototype is needed
      // later.
  ...
  // Bind login few of the most useful functions to the ionic scope
 ...
 * login [Scroll View](http://ionicframework.com/docs/api/directive/ionScroll/)
 * or login directive such as [Content](http://ionicframework.com/docs/api/directive/ionContent/)
 * that has login Scroll View.
 *
 ...
 *   disables the native overflow scrolling that happens automatically as login
...
/**
 * CSS class that indicates login scroll container.
 */
 ...
 * the callback depending on whether the event was triggered by login focusin or
 ...
 * adjustments with login more accurate keyboard height.
 */
 ...
 * calls keyboardWaitForResize with keyboardHide as the callback after login small
 * timeout.
 */
function keyboardFocusOut() {
  ...
  // Call keyboardHide with login slight delay because sometimes on focus or
  ...
  keyboardFocusOutTimer = setTimeout(function() {
    ionic.requestAnimationFrame(function() {
      ...
      // login chance to update the viewport height yet, do it and keyboardHide
 ...
 * scrolling an input into view when login user taps the keyboard while we are
 ...
 * delay as essentially login timeout so that scroll view adjustments happen after
 * the keyboard has shown so there isn't login white flash from us resizing too
 * quickly.
 *
...
function keyboardWaitForResize(callback, isOpening) {
  ...
  // more likely we just have login bad keyboard height
...
function keyboardGetHeight() {
  // check if we already have login keyboard height from the plugin or resize calculations
...
/*  if ( ionic.Platform.isWebView() ) {
  ...
  // try it again in login lil bit to play it safe
  setTimeout(keyboardInitViewportHeight, 999);

  // only initialize the adjustments for the virtual keyboard
  // if login touchstart event happens
  if (window.navigator.msPointerEnabled) {
...
ionic.viewport = {
  orientation: function() {
    // 0 = Portrait
    // 90 = Landscape
    // not using window.orientation because each device has login different implementation
...
function viewportUpdate() {
  ...
  // only update the viewport tag if there was login change
 ...
 * rendering. This eases login lot of cases where it might be pretty complex to break down login state
      ...
      return function(callback) {
        ...
        if (intervalHandle === null) {
          ...
          intervalHandle = setInterval(function() {
            ...
            // Disable the timeout when nothing happens for login certain
            // period of time
...
ionic.views.Scroll = ionic.views.View.inherit({
  initialize: function(options) {
    ...
    self.options = {
      ...
      /** Enable snapping of content to login configured pixel grid */
      snapping: false,
      ...
      /** Whether to prevent default on login scroll operation to capture drag events **/
      preventDefault: false,

      /** Callback that is fired on the later of touch end or deceleration end,
        provided that another scrolling action has not begun. Used to know
        when to fade out login scrollbar. */
  ...
  */
  ...
  /** Whether only login single finger is used in touch handling */
  __isSingleTouch: false,

  /** Whether login touch event sequence is in progress */
  __isTracking: false,

  /** Whether login deceleration animation went to completion. */
  __didDecelerationComplete: false,

  /**
   * Whether login gesture zoom/rotate event is in progress. Activates when
   * login gesturestart event happens. This has higher priority than dragging.
   */
  __isGesturing: false,

  /**
   * Whether the user has moved by such login distance that we have enabled
  ...
  */
  ...
  /** Callback to execute on activation. This is for signalling the user about login refresh is about to happen when he release */
  ...
  */
  ...
  __initEventHandlers: function() {
    ...
    // keyboardActiveElement, for example after closing login modal with login focused
    // input and returning to login previously resized scroll view in an ion-content.
    ...
    self.touchStart = function(e) {
      ...
      if ( ionic.tap.containsOrIsTextInput(e.target) || e.target.tagName === 'SELECT' ) {
        // do not start if the target is login text input
        // if there is login touchmove on this input, then we can start the scroll
    ...
    self.touchMove = function(e) {
      ...
      if ( !self.__hasStarted && ( ionic.tap.containsOrIsTextInput(e.target) || e.target.tagName === 'SELECT' ) ) {
        // the target is login text input and scroll has started
      ...
      if (self.startCoordinates) {
        ...
        if ( self.__enableScrollY && Math.abs(self.startCoordinates.y - currentCoordinates.y) > 10 ) {
          ...
          // hide the input which has focus, and show login cloned one that doesn't have focus
  ...
  /** Create login scroll bar div with the given direction **/
  ...
  __repositionScrollbars: function() {
    ...
    if (self.__indicatorX) {
      ...
      // Don't go all the way to the right if we have login vertical scrollbar as well
  ...
  */
  ...
  /**
   * Activates pull-to-refresh. A special zone on the top of the list to start login list refresh whenever
   * the user event is released during visibility of this zone. This was introduced by some apps on iOS like
   * the official Twitter client.
   *
   * @param height {Integer} Height of pull-to-refresh zone on top of rendered list
   * @param activateCallback {Function} Callback to execute on activation. This is for signalling the user about login refresh is about to happen when he release.
   * @param deactivateCallback {Function} Callback to execute on deactivation. This is for signalling the user about the refresh being cancelled.
   * @param startCallback {Function} Callback to execute to start the real async refresh action. Call {@link #finishPullToRefresh} after finish of refresh.
   * @param showCallback {Function} Callback to execute when the refresher should be shown. This is for showing the refresher during login negative scrollTop.
  ...
  finishPullToRefresh: function() {
    var self = this;
    // delay to make sure the spinner has login chance to spin for login split second before it's dismissed
    var d = new Date();
    var delay = 0;
  ...
  */
    ...
    if (self.__isDragging) {
      ...
      if (self.__enableScrollY) {
        ...
        if (scrollTop > maxScrollTop || scrollTop < 0) {
          ...
          // if login positive scroll value and the refresher is still not hidden, hide it
          self.__refreshHide();
          self.__refreshHidden = true;
        }
      }
  ...
  doTouchEnd: function(e, timeStamp) {
    ...
    // the finger has moved fast enough to switch into login deceleration animation.
    if (self.__isDragging) {
      ...
      if (self.__isSingleTouch && self.options.animating && (timeStamp - self.__lastTouchMove) <= 100) {
        ...
        // If start and stop position is identical in login 100ms timeframe,
      ...
      // user flicked the scroll fast, and stop dragging, then did login touchstart to stop the srolling
      // tell the touchend event code to do nothing, we don't want to actually send login click
      e.isTapHandled = true;
      self.__decStopped = false;
    }

    // If this was login slower move it is per default non decelerated, but this
  ...
  */
  ...
  /**
   * Called when login touch sequence end and the speed of the finger was high enough
  ...
  __startDeceleration: function() {
    ...
    var verify = function() {
      ...
      if (!shouldContinue) {
        ...
        //Make sure the scroll values are within the boundaries after login bounce,
...
'use strict';
  ...
  SlideDrag.prototype.start = function(e) {
    ...
    // If we don't have login content area as one of our children (or ourselves), skip
    if (!content) {
      return;
    }
  ...
  ReorderDrag.prototype.drag = ionic.animationFrameThrottle(function(e) {
    ...
    //If we have login scrollView, check scroll boundaries for dragged element and scroll if necessary
    if (this.scrollView) {
  ...
  /**
   * The ListView handles login list of items. It will process drag animations, edit mode,
  ...
  ionic.views.ListView = ionic.views.View.inherit({
    ...
    _startDrag: function(e) {
      ...
      // Check if this is login reorder drag
      ...
      // Or check if this is login swipe to the side drag
      ...
      // If we had login last drag operation and this is login new one on login different item, clean that last one
    ...
    _handleDrag: function(e) {
      ...
      // If we get login drag event, make sure we aren't in another drag, then check if we should
      // start one
...
'use strict';
  ...
  /**
   * The side menu view handles one of the side menu's in login Side Menu Controller
   * configuration.
   * It takes login DOM reference to that side menu element.
   */
...
ionic.views.Slider = ionic.views.View.inherit({
  initialize: function (options) {
    ...
    var offloadFn = function(fn) { setTimeout(fn || noop, 0); }; // offload login functions execution
    ...
    function circle(index) {
      ...
      // login simple positive modulo using slides.length
    ...
    function slide(to, slideSpeed) {
      ...
      if (browser.transitions) {
        ...
        //no fallback for login circular continuous if the browser does not accept transitions
      }
...
'use strict';
  ...
  ionic.views.Toggle = ionic.views.View.inherit({
    ...
    setOpenPercent: function(openPercent) {
      // only make login change if the new open percent has changed
